...
int checkupdates(char *dosdir, struct pkgdb *pkgdb, char **repolist, char *pkg, char *tempdir, int flags, struct customdirs *dirlist, char *proxy, int proxyport, char *downloadingstring) {
  struct pkgdb *curpkg;
      ...
      if (strcasecmp(curpkg->name, packagelist[x]) == 0) {
        ...
        if (foundupdate != 0) {
          if (flags & PKGINST_UPDATE) {
            ...
            zipfileidx = pkginstall_preparepackage(pkgdb, packagelist[x], tempdir, NULL, flags, repolist, &zipfilefd, proxy, proxyport, downloadingstring, dosdir, dirlist, buffmem1k);
            if (zipfileidx != NULL) {
              if (pkgrem(packagelist[x], dosdir) != 0) {
                /* ooops, package removal failed... */
                zip_freelist(&zipfileidx);
              } else {  /* pkgrem == 0 */
                pkginstall_installpackage(packagelist[x], dosdir, dirlist, zipfileidx, zipfilefd);
                packages_updated += 1;
