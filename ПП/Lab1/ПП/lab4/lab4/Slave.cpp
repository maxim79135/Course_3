...
vector<State*> thread_func2(Map * map, State * min, BinTree * close, BinTree * open, int child[4]) {
	vector<State*> lol;
	int flag_solution = 0;
	int local_flag_solution = 0;
	int child_with_parent[5] = { 0,0,0,0,0 };
	child_with_parent[0] = pvm_parent();
...
vector<State*> aPar2(Map * map, int side, int child[4]) {
	...
	if (flag_active == true)
	{
		resultP2 = thread_func2(open->min()->getMap(), open->min(), close, open, child);
	}
	//else
...
int main(int argc, char** argv)
{
	...
	pvm_recv(     /* получить от родительской задачи значения m и blksize. */
		...
		SIZETAG                     /* идентификатор ожидаемого сообщения */
	);
	pvm_upkint(&lines, 1, 1);       /* распаковать принятые значения m */
	pvm_upkint(&cols, 1, 1); /* ... и blksize */
